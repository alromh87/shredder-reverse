...
#define SHRED_STOP 0    // Shredding state Stopped
#define SHRED_FORWARD 1 // Shredding state Forward
#define SHRED_REVERSE 2 // Shredding state Reversing
...
void loop()
{
  ...
  if (!alarmed)
  {
    ...
    {
    case JAMMED_YE:
      if ((current <= runConf.v0A + 2) || (millis() > jamTick + 2000))
      {            // wait a bit to make the shredder halt
      ...
      break;
    case JAMMED_RV:
      if (millis() > jamTick + runConf.unjamReverseT)
      { // Reverse for set amount and stop
        halt();
        jamTick = millis();
        jamState = JAMMED_RE;
      }
      break;
    case JAMMED_RE:
      if ((current <= runConf.v0A + 2) || (millis() > jamTick + 3000))
      { // Let the motor come to a halt
        jamState = JAMMED_NO;
...
void countJams()
{
  ...
  if (jamTime < (runConf.minJamTime))
  {                       //if that is inside unacceptable limits
  ...
  { //If it has been ages since the last jam
    startTime = millis();
...
void readConfig()
{
  ...
  else
  {
    ...
    Serial.print("0A analogRead offset: ");
    Serial.println(runConf.v0A);
    Serial.print("@ start wait for ");
    Serial.print(runConf.startSpan);
    Serial.println("ms before detecting jams");
    Serial.print("Alarm in case of ");
    Serial.print(runConf.maxJams);
    Serial.print(" jams detected within ");
    Serial.print(runConf.minJamTime);
    Serial.println("ms");
    Serial.print("Reverse for ");
    Serial.print(runConf.unjamReverseT);
    Serial.println("ms to unjam");
    Serial.print("Detect jam when current over ");
    Serial.println(AREAD_2_A(runConf.maxCurrent));
#endif
  }
#ifdef DEBUG
  if (ver != EEPROM_V)
...
void printBar()
{                                                                        //displays the drawn current as a bar
...
void printValue()
{ //displays the drawn current as a value
  lcd.setCursor(0, 1);
  if (current < 1000)
    lcd.print(" ");
  lcd.print(current);
